...
{
    while (len--)
        crc16val = crc16_byte(crc16val, *buffer++);
    return crc16val;
}
...
const uint32_t crc32_mpeg_table[256] = {
...
uint32_t crc32_mpeg2(const uint8_t *data, int len)
{
    ...
    while (len--)
    {
        crc = (crc << 8) ^ crc32_mpeg_table[((crc >> 24) ^ *data++) & 0xff];
    }
    return crc;
}

const uint16_t crc16_ccitt_false_table[] = {
...
uint16_t crc16_ccitt_false(const uint8_t *data, int len)
{
    ...
    while (len--)
    {
        crc = (crc << 8) ^ crc16_ccitt_false_table[(crc >> 8) ^ *data++];
    }
    return crc;
}
...
const uint8_t crc8_maxim_table[] = {
