 ...
 * @version: 
 * @Author: zhouhao
 * @Date: 2024-07-11 09:19:13
 * @LastEditors: zhouhao
 * @LastEditTime: 2024-07-11 13:42:56
 */
...
#define QL_FOTA_HTTP_LOG(msg, ...) QL_LOG(QL_FOTA_HTTP_LOG_LEVEL, "lt_FOTA_http", msg, ##__VA_ARGS__)
#define QL_FOTA_HTTP_LOG_PUSH(msg, ...) QL_LOG_PUSH("lt_FOTA_http", msg, ##__VA_ARGS__)
...
//#define HTTP_DLOAD_URL                      "http://ciitlootom.top:28430/api/v3/file/download/tF63x1IKMeq3uDa1?sign=El1EUt14u0Of3F3DeORgOKWvP07l4qcnX3yFsyqS1f0%3D%3A1695098838"
#define HTTP_DLOAD_URL						"http://218.90.141.146:28430/upgrade/out.pac"                      
...
typedef struct 
{
    ...
    char fota_url[QL_FOTA_PACK_NAME_MAX_LEN];	//下载到本地的升级文件的位置
...
int fota_http_init(fota_http_client_t* fota_http_cli_p)
{
	...
	fota_http_cli_p->i_save_size = 0;
	if(ql_sdmmc_mount()==0)//存在sd卡就用sd升级。
	{
		memcpy(fota_http_cli_p->fota_packname,"SD:fota.pack",strlen("SD:fota.pack"));
	}
	else
	{
		memcpy(fota_http_cli_p->fota_packname,"UFS:fota.pack",strlen("UFS:fota.pack"));	//默认升级包地址
	}
	//memcpy(fota_http_cli_p->fota_packname,"SD:fota.pack",strlen("SD:fota.pack"));	//默认升级包地址
...
static ql_datacall_errcode_e fota_http_active(uint8_t sim,int cid)
{
	...
	//ret = ql_get_data_call_info(sim, cid, &info);
	//if(ret==0)
	//{
		//ret=QL_DATACALL_SUCCESS;
		//return ret;
	//}
...
static int fota_http_evn_request(fota_http_client_t* fota_http_cli_p)
{
	...
	ql_httpc_setopt(&(fota_http_cli_p->http_cli), HTTP_CLIENT_OPT_URL, fota_http_cli_p->fota_url);
	//设置sim_id
...
ql_fota_result_e  fota_http_result_process(void)
{
	...
	{
		QL_FOTA_HTTP_LOG("fota ql_fota_get_result failed ");
		return QL_FOTA_STATUS_INVALID;
	}

	if ( p_fota_result == QL_FOTA_FINISHED )
	{
		QL_FOTA_HTTP_LOG("fota update finished");
...
void fota_http_app_thread()
{
	...
	//获取升级结果
	ret=fota_http_result_process();
	if(ret == QL_FOTA_FINISHED)
	{
		goto exit;
	}
	//else if(ret ==QL_FOTA_STATUS_INVALID)
	//{
		//ql_rtos_task_sleep_s(20);
	//}

	ql_dev_get_firmware_version(version_buf, sizeof(version_buf));
	QL_FOTA_HTTP_LOG("fota current version:  %s", version_buf);
	...
	while( ui_down_times-- )
	{
		...
		{
			//下载完成
			QL_FOTA_HTTP_LOG("fota packge is ok.");
			break;
		}
...
void lt_fota_http_app_thread(void *argv)
{
	QL_FOTA_HTTP_LOG("lt_fota_http_app_thread");
	char *downloadurl=(char *)argv;
	ql_fota_result_e ret;
	ql_rtos_task_sleep_s(5);
	#ifdef QL_APP_FEATURE_SDMMC
    QL_FOTA_HTTP_LOG("ql sdmmc mount : %d",ql_sdmmc_mount());
	...
	memcpy(fota_http_cli.fota_url,downloadurl,strlen(downloadurl));

//

	QL_FOTA_HTTP_LOG("fota_url:%s",fota_http_cli.fota_url);
...
static char fota_url[255]={0};
void lt_ql_fota_start(char *url)
{
	memset(fota_url,0,255);
	memcpy(fota_url,url,strlen(url));
	QlOSStatus err = QL_OSI_SUCCESS;
	err = ql_rtos_task_create(&fota_http_task, 4096*32, APP_PRIORITY_NORMAL, "QfotaHttp", lt_fota_http_app_thread, fota_url, 5);
    if (err != QL_OSI_SUCCESS)
    {
        QL_FOTA_HTTP_LOG("created task failed");
    }
